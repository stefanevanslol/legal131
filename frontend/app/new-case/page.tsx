...
export default function NewCasePage() {
    ...
    const handleGenerate = async () => {
        ...
        try {
            ...
            a.href = url;
            a.download = `Demand_Letter_${data.clientName || 'Draft'}.docx`;
            document.body.appendChild(a);
            a.click();
            a.remove();

            setStep(3); // Move to download success step
        } catch (err) {
            console.error(err);
                    ...
                    {step === 2 && (
                        <div className="flex flex-col items-center justify-center py-12 text-center animate-in fade-in slide-in-from-bottom-4">
                            ...
                            <p className="text-slate-500 mb-8 max-w-sm">
                                We have successfully extracted data for <strong>{data?.clientName || 'the client'}</strong>.
                                Click below to generate the demand letter.
                            </p>
                            <button
                                onClick={handleGenerate}
                                className="bg-blue-600 text-white px-8 py-3 rounded-lg font-medium hover:bg-blue-700 transition-colors shadow-lg flex items-center gap-2"
                            >
                                <FileText size={18} />
                    ...
                    {step === 3 && (
                        <div className="flex flex-col items-center justify-center py-12 text-center animate-in fade-in slide-in-from-bottom-4">
                            ...
                            <button
                                ...
                                onClick={() => { setStep(1); setData(null); setFiles([]); }}
                            >
                                Start New Case
                            </button>
                        </div>
